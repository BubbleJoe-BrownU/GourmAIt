...
def init_model(init_from, device, to_compile):
    epoch_num = 0
    best_val_loss = float('inf')
    ...
    return model, optimizer, epoch_num, best_val_loss
...
def train(model, optimizer, epoch_num, max_epochs, best_val_loss, decay_lr, learning_rate, out_dir):
...
def main():
    # parser = argparse.ArgumentParser(description='overwrite default arguments')
    # parser.add_argument('--init_from', default='scratch')
    ...
    epoch_num = 0
    best_val_loss = float('inf')
    model, optimizer, epoch_num, best_val_loss = init_model(init_from)
    ...
    train(model, optimizer, epoch_num, max_epochs, best_val_loss, decay_lr, learning_rate, out_dir)